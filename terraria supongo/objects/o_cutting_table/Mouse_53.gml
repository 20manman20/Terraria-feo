...
if _valh != -1 {
	if !cut_x[_valh] {
		cut_x[_valh]	= true
		var _cut	= instance_create_depth(x-50+50*_valh-cut_w/4,y-100,depth,o_cuts)
		_cut.image_xscale	= cut_w/2
...
if _valv != -1 {
	if !cut_y[_valv] {
		cut_y[_valv]	= true
		var _cut	= instance_create_depth(x-100,y-50+50*_valv-cut_w/4,depth,o_cuts)
		_cut.image_xscale	= cut_table_r*2
		_cut.image_yscale	= cut_w/2
	} else {
		cut_y[_valv]	= false
